@extends('layouts.app')
@section('title', __('common/title.okr.show', ['name' => $user->name]))
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('common/title.okr.show', ['name' => $user->name]) }}</div>
                <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach($okrs as $okr)
                                            <tr>
                                                ...
                                                <td>{{ $okr->okr }}</td>
                                                <td>{{ $okr->score }}</td>
                                                <td>{{ link_to_route('objective.show', __('common/action.detail'), $okr->id); }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
